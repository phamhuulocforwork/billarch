...
from vars import BILLARCH_THEMES, OOMOX_TEMPLATES, OOMOX_COLORS, BASE_CONFIGS, HOME, SESSION_TYPE
...
class CopyOption(BaseOption):
	...
	def _run(self, theme_name: str) -> None:
		cfg_path = BILLARCH_THEMES / theme_name / self.name
...
class TmuxCfgOption(BaseOption):
	...
	def _run(self, theme_name: str) -> None:
		...
		custom_prefs: Path = BILLARCH_THEMES / theme_name / self.name
...
class FishOption(BaseOption):
	...
	def _run(self, theme_name: str) -> None:
		...
		if cfg_path.exists() and cfg_path.is_file():
			...
			if self.apply_theme:
				subprocess.Popen(
					['fish', '-c', 'echo "y" | fish_config theme save billarch'],
					stdout=subprocess.PIPE,
					stderr=subprocess.PIPE,
					text=True
				)
...
class GTKOption(BaseOption):
	...
	def _run(self, theme_name: str) -> None:
		...
		theme_name: str = f"billarch-{theme_name}"
