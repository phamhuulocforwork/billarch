...
from vars import (
	SESSION_TYPE, BILLARCH_DIR, BILLARCH_CONFIG,
	WALLPAPER_SYMLINC, BILLARCH_ASSETS
)


class Config:
	__slots__ = ()
	symlink_wallpapers = Path.home() / ".config" / "billarch" / "current_wallpaper"
	...
	def __load_yaml(cls) -> dict:
		...
		"""
		if not BILLARCH_CONFIG.exists():
			raise NoConfigFile()

		with open(BILLARCH_CONFIG, 'r') as f:
	...
	def __dump_yaml(cls, data: dict) -> None:
		...
		with open(BILLARCH_CONFIG, 'w') as f:
			yaml.dump(data, f)
