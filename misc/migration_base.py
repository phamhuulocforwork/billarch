...
def check_called_from_update_script() -> None:
    """
    Fail if the script was not launched by update-billarch.

    update-billarch sets BILLARCH_UPDATE_RUNNER=1 before calling python3,
    so running the migration directly (python migrate_to_X.py) is blocked.
    """
    if not os.environ.get("BILLARCH_UPDATE_RUNNER"):
        fail(
            "This migration script must be run through 'update-billarch', not directly.\n"
            "Use: update-billarch update"
        )


def check_billarch_tools_updated() -> None:
    """
    Fail if the billarch-tools package has a pending update.
...
class GitHubSync:
    ...
    Example:
        gh = GitHubSync(repo="billarch/billarch", tag="3.1.0")
    ...
    _USER_AGENT_PREFIX = "billarch-migrator"
